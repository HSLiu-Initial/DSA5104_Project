...
def train_validate_deepfm(args, model_save_dir, model, criterion, optimizer, decay, train_loader, train_writer,
                          test_loader,
            ...
            for step, batch in enumerate(train_loader):
                ...
                if args.use_mix_precision:
                    with autocast(enabled=args.use_mix_precision):  #
                        logits = model(sparse_feature, hist_item, hist_valid_lens, use_emb=True)
                        # compute loss
...
def test_deepfm(model, model_save_dir, test_loader, criterion, args):
    model.eval()
    with torch.no_grad():
        if args.use_ddp is True:
