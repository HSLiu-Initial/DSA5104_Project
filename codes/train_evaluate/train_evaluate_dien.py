...
def train_validate_dien(args, model_save_dir, model, criterion, optimizer, decay, train_loader, train_writer,
                        test_loader,
    ...
    for epoch in range(start_epoch, args.epochs):
        ...
        with tqdm(total=len(train_loader), dynamic_ncols=True) as t:
            for step, batch in enumerate(train_loader):
                ...
                neg_hist_item = batch['neg_hist_item'].cuda()
                hist_valid_lens = batch['hist_valid_lens'].cuda()
                dense_feature = batch['dense_feature'].cuda()
                ...
                if args.use_mix_precision:
                    with autocast(enabled=args.use_mix_precision):  #
                        logits, aux_loss = model(target_item, hist_item, neg_hist_item, hist_valid_lens, dense_feature,
                                                 sparse_feature)
                        # compute loss
                    ...
                    # compute loss
                    logit_loss = criterion(logits, y.float())
                tr_loss = logit_loss + args.coef_aux * aux_loss
                ...
                if step % 10 == 0 and ((args.use_ddp is True and args.rank == 0) or args.use_ddp is False):
                    train_writer.add_scalar("aux_loss", aux_loss.item(), train_iter)
        ...
        with torch.no_grad():
            ...
            with tqdm(total=len(test_loader), dynamic_ncols=True) as t:
                for step, batch in enumerate(test_loader):
                    ...
                    if args.use_mix_precision:
                        with autocast(enabled=args.use_mix_precision):  #
                            logits, aux_loss = model(target_item, hist_item, neg_hist_item, hist_valid_lens,
                                                     dense_feature,
                                                     sparse_feature)
                            # compute loss
            ...
            validate_aux_loss = np.mean(validate_aux_loss) if not args.use_ddp else torch.tensor(
                np.mean(validate_aux_loss) / args.world_size, device=val_auc.device)
            ...
            if args.use_ddp:
                ...
                dist.reduce(validate_aux_loss, 0, op=dist.ReduceOp.SUM)  #
        ...
        if (args.use_ddp is True and args.rank == 0) or args.use_ddp is False:
            ...
            eval_writer.add_scalar("eval_aux_loss", validate_aux_loss, train_iter)
...
def test_dien(model, model_save_dir, test_loader, criterion, args):
    model.eval()
    with torch.no_grad():
        if args.use_ddp is True:
        ...
        with tqdm(total=len(test_loader), dynamic_ncols=True) as t:
            for step, batch in enumerate(test_loader):
                ...
                if args.use_mix_precision:
                    ...
                    # compute loss
                    logit_loss = criterion(logits, y.float())
                loss = logit_loss + args.coef_aux * aux_loss
