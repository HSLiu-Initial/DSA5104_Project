...
def timestr2timestamp(tstr, time_format):
    temp = datetime.datetime.strptime(tstr, time_format)
    if temp.year < 1970:
        fix_date = temp.replace(year=2023)
    else:
        fix_date = temp
    return int(time.mktime(fix_date.timetuple()))
...
def split_tabular(tabular_file,
    ...
    logging.info('time points for cutting search pool, target train and target test is {} and {}'.format(split_tps[0],
                                                                                                         split_tps[1]))
    ...
    num_file = sum([1 for i in open(tabular_file)])
    ...
    target_train_lines_dump = copy.copy(target_train_lines)
    target_test_lines_dump = copy.copy(target_test_lines)
    ...
    if neg_sample_flag:
        ...
        cnt = 0
        # num_file = len(target_train_lines)
...
if __name__ == "__main__":
    ...
    if dataset == "tmall":
        # call functions
        split_tabular(os.path.join(root_dir_path, cnf.get(dataset, 'remapped_tabular_file')),
                      ...
                      cnf.get(dataset, 'sampling_c_pos_list_idcat'),
                      cnf.get(dataset, 'time_format'),
                      os.path.join(root_dir_path, cnf.get(dataset, 'search_pool_file')),
                      os.path.join(root_dir_path, cnf.get(dataset, 'target_train_file')),
                      os.path.join(root_dir_path, cnf.get(dataset, 'target_test_file')),
                      os.path.join(root_dir_path, cnf.get(dataset, 'sync_seq_dict_file')),
    ...
    elif dataset == "alipay" or dataset == "taobao":
        split_tabular(os.path.join(root_dir_path, cnf.get(dataset, 'remapped_tabular_file')),
                      os.path.join(root_dir_path, cnf.get(dataset, 'sampling_collection_file')),
                      cnf.getboolean(dataset, 'is_timestamp_flag'),
                      cnf.getboolean(dataset, 'neg_sample_flag'),
    ...
    sampling(os.path.join(root_dir_path, cnf.get(dataset, 'target_train_file')),  #
             os.path.join(root_dir_path, cnf.get(dataset, 'target_train_sample_file')),
             rate=cnf.getfloat(dataset, 'target_sample_rate'))
    sampling(os.path.join(root_dir_path, cnf.get(dataset, 'target_test_file')),
             os.path.join(root_dir_path, cnf.get(dataset, 'target_test_sample_file')),
             rate=cnf.getfloat(dataset, 'target_sample_rate'))
