...
if __name__ == "__main__":
    ...
    sync_seq_dict_file = cnf.get(dataset, 'sync_seq_dict_file')
        ...
        for line in tqdm(search_pool_lines,
            ...
            if line_split[-1] == '0':
                ...
                if sync_id in sync_seq_dict:  #
                    seq_part = ','.join(np.array(line_split)[sampling_c_pos_list].tolist())
                    sync_seq_dict[sync_id].append((seq_part, int(float(line_split[timestamp_pos]))))
                else:
    ...
    dump_pkl(os.path.join(root_dir_path, sync_seq_dict_file), sync_seq_dict)
