...
class ESReader(object):
    def __init__(self,
        ...
        self.size = size  # retrieval size

    # For UBR
    def query_ubr(self, queries, sync_ids):
        ms = MultiSearch(using=self.es, index=self.index_name)
        for i, q in enumerate(queries):
            s = Search().filter("terms", sync_id=[sync_ids[i]]).filter("terms", label=['1']).query("match", line=q)[
                1:self.size + 1]
            ms = ms.add(s)
...
class queryGen(object):
    def __init__(self,
        ...
        self.query_data = self.target_data[self.query_c_pos].apply(lambda l: ','.join(map(str, l)),
                                                                   1).values  #
        ...
        self.step = 0
        logging.info(f"data loaded.file is {target_file}")
        # print('data loaded')
