...
from codes.model.deepfm import DeepFM
from codes.train_evaluate.train_evaluate_deepfm import train_validate_deepfm, test_deepfm
...
def main(args):
    ...
    try:
        ...
        if args.use_ddp is True:
            raise Exception(f"use_ddp is {args.use_ddp},no use DDP")
    # if args.
    if args.use_ddp:
    ...
    model = DeepFM(dataset_summary_file=os.path.join(data_path, args.dataset, 'feateng_data',
                                                     'summary_dict.pkl'),
                   feature_num=sparse_len,
                   embed_size=args.embed_size,
                   item_feature_num=len(hist_itemfeature_dict_embedsize.keys()),
                   use_fm=True,
                   dnn_hidden_units=(200, 80), dnn_dropout=args.dnn_dropout, init_std=args.init_std)
    ...
    train_validate_deepfm(args=args, model_save_dir=model_save_dir,
    ...
    test_deepfm(model=model, model_save_dir=model_save_dir, test_loader=test_loader, criterion=criterion,
                args=args)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--init_std", type=float, help="Dropout.")
    ...
    if opts.config is not None:
        ...
        with open(os.path.join(file_dir_path,
                               'configs/hyper_sequential_deepfm.json'), 'r',
                  encoding='UTF-8') as f:  #
            options = json.load(f)
            args = ConfigParser(options)
    args.update(opts)
